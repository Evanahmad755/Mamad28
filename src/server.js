...
const { oauthEnabled, getAuthorizeUrl, exchangeCodeForUser } = require('./discordOAuth');
...
// ---------- LOGIN & DAFTAR LEWAT DISCORD OAUTH ----------
// Satu redirect URI dipakai buat DUA tujuan (login & register), dibedain
// lewat "purpose" di dalam state JWT — biar user cuma perlu daftarin SATU
// Redirect URI di Discord Developer Portal, gak usah dua-dua.
//
// /auth/discord/start        -> alur DAFTAR akun baru
// /auth/discord/login/start  -> alur LOGIN (cocokin Discord ID ke UCP yang udah ada)
app.get('/auth/discord/start', (req, res) => {
  if (!oauthEnabled) return res.status(503).send('Fitur daftar via Discord belum di-setup admin.');
  const state = jwt.sign({ purpose: 'oauth_register' }, process.env.JWT_SECRET, { expiresIn: '10m' });
  res.redirect(getAuthorizeUrl(state));
});

app.get('/auth/discord/login/start', (req, res) => {
  if (!oauthEnabled) return res.status(503).send('Fitur login via Discord belum di-setup admin.');
  const state = jwt.sign({ purpose: 'oauth_login' }, process.env.JWT_SECRET, { expiresIn: '10m' });
  res.redirect(getAuthorizeUrl(state));
});

app.get('/auth/discord/callback', async (req, res) => {
  if (!oauthEnabled) return res.status(503).send('Fitur Discord belum di-setup admin.');
  const { code, state } = req.query;
  if (!code || !state) {
    return res.redirect('/login.html?error=' + encodeURIComponent('Sesi tidak valid, coba ulangi.'));
  }
  let statePayload;
  try {
    statePayload = jwt.verify(state, process.env.JWT_SECRET);
  } catch (err) {
    return res.redirect('/login.html?error=' + encodeURIComponent('Sesi kadaluarsa, coba ulangi.'));
  }
  ...
  try {
    ...
    return res.redirect('/login.html?error=' + encodeURIComponent('Gagal verifikasi Discord, coba lagi.'));
  }

  // ---- Alur LOGIN ----
  if (statePayload.purpose === 'oauth_login') {
    try {
      const [rows] = await pool.query('SELECT ID, ucp FROM playerucp WHERE DiscordID = ? LIMIT 1', [discordUser.id]);
      if (rows.length === 0) {
        return res.redirect('/register-new.html?error=' + encodeURIComponent('Discord ini belum punya UCP. Daftar dulu di bawah.'));
      }
      const account = rows[0];
      const token = jwt.sign({ ucpId: account.ID, ucp: account.ucp }, process.env.JWT_SECRET, { expiresIn: '7d' });
      return res.redirect(`/login.html?token=${token}&ucp=${encodeURIComponent(account.ucp)}`);
    } catch (err) {
      console.error('discord login lookup error:', err);
      return res.redirect('/login.html?error=' + encodeURIComponent('Terjadi kesalahan server.'));
    }
  }

  // ---- Alur DAFTAR (default / purpose 'oauth_register') ----
  try {
    const [existing] = await pool.query('SELECT ucp FROM playerucp WHERE DiscordID = ? LIMIT 1', [discordUser.id]);
    if (existing.length > 0) {
      return res.redirect('/login.html?info=' + encodeURIComponent(`Discord kamu sudah punya UCP: ${existing[0].ucp}. Silakan login.`));
    }
    const verifyToken = jwt.sign({ purpose: 'discord_verified', discordId: discordUser.id }, process.env.JWT_SECRET, {
      expiresIn: '5m',
    });
    return res.redirect(`/register-new.html?vtoken=${verifyToken}&discordName=${encodeURIComponent(discordUser.username)}`);
  } catch (err) {
    console.error('discord oauth callback error:', err);
    return res.redirect('/register-new.html?error=' + encodeURIComponent('Gagal verifikasi Discord, coba lagi.'));
  }
});

app.post('/api/register-new', async (req, res) => {
  const { vtoken, ucp, password } = req.body || {};
  if (!vtoken) return badRequest(res, 'Sesi verifikasi Discord tidak ada, ulangi dari awal.');
  if (!ucp || !UCP_REGEX.test(ucp)) {
    return badRequest(res, 'Username UCP harus 3-22 karakter (huruf/angka/underscore).');
  }
  if (!password || password.length < 6 || password.length > 32) {
  ...
  try {
    ...
    if (payload.purpose !== 'discord_verified') throw new Error('wrong purpose');
  } catch (err) {
    return badRequest(res, 'Sesi verifikasi Discord kadaluarsa (5 menit), ulangi dari awal.');
  }

  try {
    const [existingDiscord] = await pool.query('SELECT ucp FROM playerucp WHERE DiscordID = ? LIMIT 1', [
      payload.discordId,
    ]);
    if (existingDiscord.length > 0) {
      return badRequest(res, `Discord ini sudah punya UCP: ${existingDiscord[0].ucp}. Silakan login.`);
    }
    const [existingUcp] = await pool.query('SELECT ID FROM playerucp WHERE ucp = ? LIMIT 1', [ucp]);
    if (existingUcp.length > 0) {
      return badRequest(res, 'Username UCP ini sudah dipakai, pilih yang lain.');
    }

    const salt = generateSalt();
    const hashed = hashPassword(password, salt);
    await pool.query(
      'INSERT INTO playerucp (ucp, password, salt, DiscordID, verifycode, extrac, reedem) VALUES (?, ?, ?, ?, 0, 0, 0)',
      [ucp, hashed, salt, payload.discordId]
    );
    return res.json({ ok: true, message: 'Akun UCP berhasil dibuat, silakan login.' });
  } catch (err) {
...
// Ini cuma jalur CADANGAN buat akun yang belum sempat link Discord. Jalur
// UTAMA yang aman itu "Masuk pakai Discord" (/auth/discord/login/start),
// yang gak butuh password sama sekali. OTP dicabut dari sini karena Discord
// Login udah jauh lebih kuat & gak gampang gagal kirim DM kayak OTP dulu.
app.post('/api/login', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT ID, ucp, password, salt FROM playerucp WHERE ucp = ? LIMIT 1', [ucp]);
    if (rows.length === 0) {
