...
2. Jalankan `database/002_updates_and_push.sql`
3. ~~`database/003_login_otp.sql`~~ — SUDAH GAK DIPAKAI (fitur OTP dicabut, diganti Login lewat Discord). Boleh dilewatin, gak dibutuhkan lagi.
4. (Opsional) Push Notification — generate VAPID key:
   ```bash
...
5. **Login & Daftar pakai Discord** — WAJIB biar dua tombol utama di halaman login/daftar jalan, lihat bagian di bawah
6. `npm install` ulang tiap ada dependency baru

## Login & Daftar pakai Discord (satu Redirect URI buat dua-duanya)

Cara masuk UTAMA sekarang adalah tombol **"Masuk pakai Discord"** —
gak perlu ketik username/password sama sekali. Sistem cocokin Discord ID
hasil login OAuth ke kolom `playerucp.DiscordID`:
- **Ketemu** → langsung dikasih token, masuk dashboard.
- **Gak ketemu** → diarahin ke `/register-new.html` buat bikin UCP baru (pilih username + password, akun otomatis ke-link ke Discord itu).

Form username/password lama masih ada sebagai **cadangan** (link "Masuk
manual" di bawah tombol Discord) buat akun yang belum sempat ke-link Discord
— tanpa OTP lagi (OTP dicabut karena Discord Login sendiri udah jauh lebih
kuat & gak gampang gagal kirim DM).

Setup (dari [discord.com/developers/applications](https://discord.com/developers/applications) → aplikasi bot yang sama → tab **OAuth2**, BUKAN tab Bot):
1. Catat **Client ID** dan **Client Secret** (generate kalau belum ada)
2. Di bagian **Redirects**, tambahin SATU URL ini: `https://domain-lu.vercel.app/auth/discord/callback` (ganti sesuai domain asli — dipakai bareng buat login maupun daftar, gak perlu didaftarin dua kali)
3. Isi env Vercel: `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `DISCORD_REDIRECT_URI` (harus PERSIS sama kayak langkah 2)
4. Redeploy

Kalau tiga env itu dikosongin, tombol "Masuk/Daftar pakai Discord" cuma
...
| GET | `/auth/discord/start` | Mulai alur DAFTAR via Discord OAuth |
| GET | `/auth/discord/login/start` | Mulai alur LOGIN via Discord OAuth |
| GET | `/auth/discord/callback` | Callback OAuth (dipakai dua-duanya), redirect balik ke `/login.html` atau `/register-new.html` |
| POST | `/api/register-new` | `{ vtoken, ucp, password }` — bikin UCP baru (butuh verifikasi Discord) |
| POST | `/api/login` | `{ ucp, password }` → balikin `token` (jalur cadangan) |
